...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`http://localhost:5003/product/${id}`);
      ...
      <div className={classes.wrapper}>
        <div className={classes.left}>
          <div className={classes.mainImgContainer}>
            <img
              src={`http://localhost:5003/images/${currentImage}`}
        ...
        <div className={classes.right}>
          ...
          <div className={classes.images}>
            <img
              src={`http://localhost:5003/images/${product?.firstImg}`}
              className={classes.firstPhoto}
              alt=""
              onClick={() => setCurrentImage((prev) => product?.firstImg)}
            />
            <img
              src={`http://localhost:5003/images/${product?.secondImg}`}
